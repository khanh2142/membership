...
const ContractTypeList = () => {
    ...
    const fetchData = async ({ page, limit, sortBy, sortDir }: { page: number, limit: number, sortBy?: string, sortDir?: string }) => {
        ...
        const resp = await contracttype_service.search({
            Ft_PageIndex: page - 1,
            Ft_PageSize: limit,
    ...
    const handleEdit = (checkedItems: any[]) => {
        let code = checkedItems[0].ContractTypeCode;
        setCurrentDetailCode(<ContractTypeEdit code={code}  uuid={uuid()} onSuccess={reloadList} />);
    };

    const handleAdd = () => {
        setCurrentDetailCode(<ContractTypeEdit code="" uuid={uuid()}  onSuccess={reloadList} />);
    };

    const handleDelete = (checkedItems: any[]) => {
        Confirm({
            title: _l('Delete contract type'),
            message: _l('Are you sure to delete contracttype(s): ') + checkedItems.map(c=>c.ContractTypeName).toString(),
            yes: () => {
                let listContractType = checkedItems.reduce((lstContractType: any[], currentValue : any) => {
                    lstContractType.push({
                        NetworkId: NetworkId,
                        OrgId: OrgId,
                        LocationCode: currentValue.ContractTypeCode,
                    });
                    return lstContractType;
                }, []);
                contracttype_service.remove(listContractType).then(resp => {
                    if (resp.Success) {
                        toast.success(_l('Contract type data deleted'));
                        reloadList();
                    }
                    else {
            ...
            <Container>
                <Header className="page-header border-bottom">
                    <Grid fluid>
                        <Row>
                            <Col md={8}>
                                <span className="page-title">{_l('Contract Type Lis')}</span>
                            </Col>
                            <Col md={8}>
